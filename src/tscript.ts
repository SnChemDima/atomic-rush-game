const atom_bond = new Map<string, number>();
...
const atom_types = Array<string>();
atom_types.push(...Array<string>(8).fill(atom_names[0]));
atom_types.push(...Array<string>(3).fill(atom_names[1]));
atom_types.push(...Array<string>(2).fill(atom_names[2]));
atom_types.push(...Array<string>(1).fill(atom_names[3]));

function next_atom_name(): string {
...
class Molecule {
    ...
    calculate_score() {
        let score = 0;
        for (const atom of this.atoms)
            score += (atom_bond.get(atom.cell_type) ?? -1);
...
class Game {
    ...
    constructor(canvas: HTMLCanvasElement, target_score: number, x_size: number, y_size: number) {
    ...
    try_update_cell(x_pos: number, y_pos: number, cell_type: string) {
        if (this.game_ended)
            return false;
